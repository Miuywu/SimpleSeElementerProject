...
import com.rationaleemotions.page.RawElement;
...
public final class AndroidPageObject {
     ...
     */
    @SuppressWarnings("rawtypes")
	public AndroidPageObject(WebDriver context, String jsonFileSource) {
        this.context = context;
        this.driver = (AndroidDriver) context;
        this.jsonFileSource = jsonFileSource;
    }

    /**
     * @param locale - The locale for which the page object is being created.
     * @return - The current {@link AndroidPageObject} instance.
     */
    public AndroidPageObject forLocale(String locale) {
        this.locale = locale;
        return this;
    }
    ...
    public final AndroidElement getAndroidElement(String fieldName) {
        return getElement(fieldName, AndroidElement.class);
    }
    /**
     * This method is used, when dealing with custom html elements which doesn't have the required visualisation
     * built in by our page object model. E.g., for this could include controls such as date pickers for which
    ...
    private <E> E getElement(String fieldName, Class<E> clazz) {
    	///MINH
    	By by = newRawElement(getJsonWebElement(fieldName)).getAndroidElement();
    	String aId;
    	//System.out.println(by);
    	//System.out.println(by.toString());
    	//System.out.println(by.toString().contains("accessibilityId="));
    	WebElement webElement;
		if (by.toString().contains("accessibilityId=")) {
    		aId = by.toString().substring(31, by.toString().length()-1);
    		System.out.println(aId);
    		webElement = driver.findElementByAccessibilityId(aId);
		}
		else {
