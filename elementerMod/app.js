;(function() {
  ...
  //var tabUrl = uurl
  //var tabUrl = _getCurrentTab(_returnTab)

  //import urlvar from './background.js'
  //var uurl = urlvar
  var uurl = "Wip"
  //var uurl = document.querySelector("title")
      ...
      if (!err) {
        ...
        switch (selectedElement.value) {
          case "tagId":
            ...
            if (className.value != '') {LocatorsList.push(`class=${className.value}`)
              selectedAttr = className.value}
            if (name.value != '') {LocatorsList.push(`name=${name.value}`)
              selectedAttr = name.value}
            if (tagId.value != '') {LocatorsList.push(`tagId=${tagId.value}`)
              selectedAttr = tagId.value}
            if (pageElementName.value == "locator-by-default") {pageElementName.value = selectedAttr + "_Element"}
            break
        }
